...
async function main() {
    ...
    console.log("==================================================");
    console.log(`🔬 GHOSTTRADE LIVE 100-TRADE SIMULATION (${asset})`);
    console.log("==================================================\n");
    ...
    const trades = result.tradeLog.slice(0, 100);
    
    if (trades.length < 100) {
        console.log(`Only found ${trades.length} trades! (Out of ${result.totalSignalsTaken} total)`);
        process.exit(1);
    }
    ...
    const baselineWinRate = ((baselineWins / 100) * 100).toFixed(2);
    const improvedWinRate = ((improvedWins / 100) * 100).toFixed(2);
    ...
    trades.forEach((t, i) => {
        ...
        console.log(`${(i+1).toString().padStart(3, ' ')}. Date: ${dateStr} | Entry: $${t.entryPrice.toFixed(4)} | Baseline: ${(t.baselineOutcome).padEnd(4, ' ')} | Improved: ${t.improvedOutcome}`);
    });
