...
const assets = ['AAPL', 'TSLA', 'NVDA', 'MSFT', 'META'];
const TEST_EMAIL = 'ai-tester@ghosttrade.com';

async function run() {
  console.log("🚀 STARTING 5-ASSET (GLOBAL EQUITIES) PAPER TRADE END-TO-END VALIDATION\n");
  let db;
  try {
    db = await getDb(); 
  ...
  // Clear existing paper trades for tester to prevent Portfolio Correlation Limit from blocking tests
  await db.collection('paper_trades').deleteMany({ email: TEST_EMAIL });

  for (const asset of assets) {
     // Clear DB before each asset so correlation doesn't block the next one
     await db.collection('paper_trades').deleteMany({ email: TEST_EMAIL });
