...
new_block = """  // === PHASE 4: DEEP SCAN INTERCEPTOR (Now reads from global cache) ===
  ...
  if (prompt.includes('Execute Deep Scan')) {
    ...
    if (profitableAssets.length === 0) {
      ...
      profitableAssets.forEach((s, idx) => {
        ...
        const tpPct = tp && entry ? ((abs(tp - entry) / entry) * 100).toFixed(2) : null;
        const slPct = sl && entry ? ((abs(entry - sl) / entry) * 100).toFixed(2) : null;
...
new_block = new_block.replace("abs(", "Math.abs(")
